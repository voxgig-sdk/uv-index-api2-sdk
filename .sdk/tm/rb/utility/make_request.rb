...
module UvIndexApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UvIndexApi2Response.new({})
    ...
    if fetch_err
      ...
      response = UvIndexApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UvIndexApi2Response.new(fetched)
    else
