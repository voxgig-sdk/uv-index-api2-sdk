...
class UvIndexApi2RetryFeature extends UvIndexApi2BaseFeature
{
    ...
    public function init(UvIndexApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UvIndexApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UvIndexApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UvIndexApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
