...
func TestForecastEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Forecast(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Forecast(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set UVINDEXAPI__TEST_FORECAST_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		forecastRef01Match := map[string]any{}

		forecastRef01ListResult, err := forecastRef01Ent.List(forecastRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, forecastRef01ListOk := forecastRef01ListResult.([]any)
		if !forecastRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", forecastRef01ListResult)
		}
...
func forecastBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("UVINDEXAPI__TEST_FORECAST_ENTID")
	...
	if env["UVINDEXAPI__TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewUvIndexApi2SDK(core.ToMapAny(mergedOpts))
	}
