...
[MCP](https://modelcontextprotocol.io) server exposing the UvIndexApi2 SDK as
two agent tools — `uv-index-api2_list` and `uv-index-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/uv-index-api2-mcp)
make build

# 2. Provide credentials via the environment
export UV_INDEX_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user uv-index-api2 \
  -- /absolute/path/to/uv-index-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./uv-index-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// uv-index-api2_list: first page of records
{ "entity": "forecast" }
{ "entity": "forecast", "query": { } }

// uv-index-api2_load: one record by id
{ "entity": "forecast", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/uv-index-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user uv-index-api2 \
     -- "$PWD"/dist/*/uv-index-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `uv-index-api2_list` and `uv-index-api2_load` tools now appear
   in new sessions. Ask the agent to *"list forecast using uv-index-api2"*
   and it calls `uv-index-api2_list` with `{"entity":"forecast"}`.
...
```sh
export UV_INDEX_API2_APIKEY=sk_live_xxx            # API key
export UV_INDEX_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `uv-index-api2_list` tool
...
### Call the `uv-index-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `uv-index-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `uv-index-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `UV_INDEX_API2_APIKEY` | API key sent with every request. |
| `UV_INDEX_API2_BASE` | Optional override of the API base URL. |
...
```sh
./uv-index-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"uv-index-api2_load","arguments":{"entity":"forecast","query":{"id":1}}}}'
```
