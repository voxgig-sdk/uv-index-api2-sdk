...
class TestForecastEntity:
    ...
    def test_should_stream(self):
        ...
        base = UvIndexApi2SDK.test(seed, None)
        seen = list(base.Forecast(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = UvIndexApi2SDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Forecast(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set UV_INDEX_API2_TEST_FORECAST_ENTID JSON to run live")
        client = setup["client"]
...
def _forecast_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("UV_INDEX_API2_TEST_FORECAST_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("UV_INDEX_API2_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("UV_INDEX_API2_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("UV_INDEX_API2_TEST_EXPLAIN") == "TRUE",
        "live": _live,
