...
class TestForecastDirect:
    ...
    def test_should_direct_list_forecast(self):
        setup = _forecast_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-forecast", "live" if setup["live"] else "unit")
        if _skip:
...
def _forecast_direct_setup(mockres):
    ...
    if live:
        ...
        client = UvIndexApi2SDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
