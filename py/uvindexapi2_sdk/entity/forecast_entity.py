...
from uvindexapi2_sdk.uvindexapi2_types import (
    Forecast,
    ForecastListMatch,
)
...
class ForecastEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ForecastEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Forecast]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Forecast().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
