...
// UvIndexApi2 SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        UvIndexApi2SDK $client,
        UvIndexApi2Utility $utility
    ): UvIndexApi2Context {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new UvIndexApi2Context($ctxmap, null);
    ...
    private static function fixctx(UvIndexApi2Context $ctx, UvIndexApi2SDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?UvIndexApi2Error
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new UvIndexApi2Error($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new UvIndexApi2Result([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new UvIndexApi2BaseFeature();
    ...
    {
        $hook_client = UvIndexApi2SDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends UvIndexApi2BaseFeature {
            public $hook_fn = null;
            public function TestHook(UvIndexApi2Context $ctx): void
            {
    ...
    {
        $init_client = UvIndexApi2SDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends UvIndexApi2BaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new UvIndexApi2SDK([
            'system' => [
    ...
    {
        $blocked_client = new UvIndexApi2SDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new UvIndexApi2Spec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = UvIndexApi2SDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new UvIndexApi2Operation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = UvIndexApi2SDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof UvIndexApi2Error) ? $result->err->msg : (string)$result->err;
