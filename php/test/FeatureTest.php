...
class FtCtrl extends UvIndexApi2Control
{
...
class FtClient
{
    ...
    private UvIndexApi2Utility $utility;

    public function __construct(UvIndexApi2Utility $utility, array $options)
    {
    ...
    public function get_utility(): UvIndexApi2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UvIndexApi2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UvIndexApi2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UvIndexApi2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UvIndexApi2Config::make_feature($name);
    ...
    private static function buildUrl(UvIndexApi2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UvIndexApi2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UvIndexApi2Response($fetched);
        }
    ...
    private function fail(UvIndexApi2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UvIndexApi2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
