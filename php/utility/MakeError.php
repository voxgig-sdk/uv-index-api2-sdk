...
class UvIndexApi2MakeError
{
    public static function call(?UvIndexApi2Context $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UvIndexApi2Context([], null);
        }
        $op = $ctx->op ?? new UvIndexApi2Operation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UvIndexApi2Result([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UvIndexApi2Error) ? $err->msg : (string)$err;
        ...
        $sdk_err = new UvIndexApi2Error('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof UvIndexApi2Error) {
