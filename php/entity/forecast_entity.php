...
// UvIndexApi2 SDK Forecast entity
...
class ForecastEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ForecastEntity($this->_client, $opts);
    }

    /**
     * @param Forecast|array $args Forecast data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = UvIndexApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Forecast|array The current Forecast data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Forecast fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = UvIndexApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Forecast fields).
     */
     ...
     *
     * @param ForecastListMatch|array|null $reqmatch Match filter (any subset
     *   of Forecast fields) as an assoc-array; ForecastListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Forecast[]|array A list of Forecast items as assoc-arrays at
     *   the SDK boundary; throws UvIndexApi2Error on failure (item-5 convention).
     */
