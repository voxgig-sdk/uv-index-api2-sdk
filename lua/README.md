...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Forecast()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
Create an instance: `local forecast = client:Forecast(nil)`
...
```
lua/
├── uv-index-api2_sdk.lua    -- Main SDK module
...
The main module (`uv-index-api2_sdk`) exports the SDK constructor
