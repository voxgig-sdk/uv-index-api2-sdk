...
local ForecastEntity = {}
ForecastEntity.__index = ForecastEntity


function ForecastEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ForecastEntity)
  self._name = "forecast"
  self._client = client
...
function ForecastEntity:get_name()
  return self._name
end


function ForecastEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ForecastEntity.new(self._client, opts)
end
...
function ForecastEntity:mark_deleted()
  self._deleted = true
end


function ForecastEntity:deleted()
  return true == self._deleted
end


function ForecastEntity:data_set(args)
  if args ~= nil then
...
function ForecastEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ForecastEntity:match_set(args)
  if args ~= nil then
...
function ForecastEntity:match_get()
...
function ForecastEntity:stream(action, args, callopts)
...
---@param reqmatch ForecastListMatch
---@param ctrl? table
---@return Forecast[]
---@return string? err
function ForecastEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ForecastEntity:_run_op(ctx, post_done)
  local utility = self._utility
