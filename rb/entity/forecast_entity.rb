...
class ForecastEntity
  ...
  def make
    opts = @_entopts.dup
    ForecastEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UvIndexApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = UvIndexApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [ForecastListMatch, Hash, nil] match filter (any subset of
  #   Forecast fields); defaults to nil, treated as an empty match that lists all.
