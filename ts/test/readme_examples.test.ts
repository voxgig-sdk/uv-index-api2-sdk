...
//     (\`new UvIndexApi2SDK(...)\` / \`UvIndexApi2SDK.test(...)\` become
//     \`UvIndexApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"forecast":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UvIndexApi2SDK(...)\` and any
// \`UvIndexApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UvIndexApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UvIndexApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
