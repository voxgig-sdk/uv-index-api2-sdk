// Command UvIndexApi2-cli is a CLI + REPL for the UvIndexApi2 SDK,
...
const prompt = "uv-index-api2"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "forecast"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      UV_INDEX_API2_APIKEY (api key), UV_INDEX_API2_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: UV_INDEX_API2_APIKEY carries the API key and
	// UV_INDEX_API2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("UV_INDEX_API2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("UV_INDEX_API2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUvIndexApi2SDK(opts)
