...
boru-driven command-line client **and** interactive REPL for the UvIndexApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/uv-index-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./uv-index-api2-cli --help

# 3. Provide credentials once, via the environment
export UV_INDEX_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./uv-index-api2-cli list forecast

# 5. Override the API base URL for a single call
UV_INDEX_API2_BASE=https://api.example.com ./uv-index-api2-cli list forecast

# 6. No arguments -> interactive REPL
./uv-index-api2-cli
uv-index-api2> list forecast
uv-index-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/uv-index-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/uv-index-api2-cli list forecast
   ```
...
Run with no arguments to open a REPL (prompt `uv-index-api2>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `forecast`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/uv-index-api2-cli`. |
...
(`./uv-index-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
